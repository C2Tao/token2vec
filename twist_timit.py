...
n_size = 5
n_chan = 32
ew_trans, ew_recon, ew_sharp = 0, 1, 0
################################################################################
'''
...
def error_recon(input_feat, decoded):
    err = merge([input_feat, decoded], mode=obj_recon, output_shape = dummy_shape, name = 'err_recon')
    return err
...
x = Convolution2D(1, 3, 3, activation='tanh', border_mode='same')(x)

decoded = layer_rename(x, 'reconstructed')

err_recon = error_recon(input_feat, decoded)
err_final = error_final([err_trans, err_sharp, err_recon])

tokenizer = Model(input_feat, err_final)
...
tokenizer.fit(x_train, dummy_target(x_train),
                ...
                batch_size=n_batch,
                shuffle=True,
                validation_data=(x_train, dummy_target(x_train)),
                callbacks=[])
...
recon_err = Model(input_feat, tokenizer.get_layer("err_recon").output)
